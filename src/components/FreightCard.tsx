...
export function FreightCard({
  ...
  // Recolhe o card inteiro, deixando só o cabeçalho ("N eixos") visível.
  const [cardRecolhido, setCardRecolhido] = useState(false);
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-xl border border-line bg-panel">
      <button
        type="button"
        onClick={() => setCardRecolhido((v) => !v)}
        aria-label={cardRecolhido ? "Expandir card" : "Recolher card"}
        className={`flex w-full items-baseline justify-between bg-navy px-4 text-left ${compact ? "py-2" : "py-3"}`}
      >
        <div className="flex items-baseline gap-2">
          <div className="text-[17px] font-extrabold text-primary-foreground">{eixos} eixos</div>
          <ChevronDown
            className={`h-4 w-4 self-center text-primary-foreground/70 transition-transform ${
              cardRecolhido ? "-rotate-90" : ""
            }`}
          />
        </div>
        ...
        <div className={compact ? "px-3.5 pt-2.5 pb-3" : "px-4 pt-3.5 pb-4"}>
          ...
          {!anttRecolhido && (
            <>
              ...
              >
                Deslocamento {coef.desloc.toLocaleString("pt-BR", { minimumFractionDigits: 4 })}{" "}
                R$/km · Carga/Descarga {brl(coef.cd)}
              </div>
              <Row
                compacto={compact}
                label="Tabela ANTT"
